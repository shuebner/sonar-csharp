...
if ($project.DTE.Version -eq '14.0') {
    ...
    if (Test-Path $path) {
        $fullVersion = [version]((Get-Item $path).VersionInfo.FileVersion)
...
if ($invalidVsVersion) {
    throw 'This package can only be installed on Visual Studio 2015 Update 3 or Visual Studio 2017.'
}
